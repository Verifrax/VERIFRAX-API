# VERIFRAX Specification v2.5.0 — FINAL
...
- **The term 'evidentiary_metadata' refers only to internal consistency signals and has no legal evidentiary meaning in any jurisdiction.**
...
- **`evidentiary_metadata`**: Verification establishes factual evidence metadata about claims (informational only). **The term 'evidentiary_metadata' refers only to internal consistency signals and has no legal evidentiary meaning in any jurisdiction.**
...
**CRITICAL DISCLAIMER:** TCB validation is **schema + signature structure validation only, no interpretation**. TCB validation does not imply VERIFRAX endorses TCB issuer, trust, or authoritative trust. **Cryptographic signature verification is out of scope by design.**
...
TCB validation is **schema + signature structure validation only**:

1. Validate TCB schema structure
2. Validate signature structure (algorithm, public_key, signature fields present and correctly formatted)
3. Check revocation status (if revocation registry available)
4. **Do not** perform cryptographic signature verification (out of scope by design)
5. **Do not** interpret TCB payload semantics
6. **Do not** imply endorsement or trust
...
**Note:** TCB references are **metadata only**. They do not affect certificate hash computation. `validation_status: "schema_valid"` means schema and signature structure validation passed (not cryptographic verification), **NOT** that VERIFRAX endorses the TCB.
...
- TCB validation does **NOT** perform cryptographic signature verification (out of scope by design)
- TCB validation status means "schema and signature structure valid", **NOT** "cryptographically verified" and **NOT** "endorsed"
...
**END OF SPEC v2.5.0 FINAL (FROZEN)**
